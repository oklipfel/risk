...
class Board(object):
    ...
    def is_valid_attack_path(self, path):
        ...
        starting on one player's territory and moving trough enemy territories.
    ...
    def shortest_path(self, source, target):
        ...
        and so a shortest path is simply a valid path with
        the smallest number of territories visited.
        ...
        while deque1:
            ...
            for territory in risk.definitions.territory_neighbors[current_territory]:
                if territory not in tervis:
                    ...
                    d[territory] = dictionarycopy
                    deque1.append(territory)
                tervis.add(territory)

    def can_fortify(self, source, target):
        '''
        At the end of a turn, a player may choose to fortify a
        target territory by moving armies from a source territory.
        In order for this to be a valid move,
        there must be a valid path between the source and
        target territories that is owned entirely by the same player.
        ...
        Returns:
            bool: True if reinforcing the
            target from the source territory is a valid move
        '''
        d = {}
        d[source] = [source]
        deque1 = deque()
        ...
        while deque1:
            ...
            if current_territory == target:
                return True
            a = self.friendly_neighbors(current_territory)
            good_ter_ids = [territory.territory_id for territory in a]
            for territory in good_ter_ids:
                if territory not in tervis:
                    dictionarycopy = copy.deepcopy(d[current_territory])
                    dictionarycopy.append(territory)
                    d[territory] = copy.deepcopy(d[current_territory])
                    deque1.append(territory)
                tervis.add(territory)
        return False
    ...
    def cheapest_attack_path(self, source, target):
        ...
        a cheapest valid attack path between two territories if such
        a path exists.
        ...
        Returns:
            [int]: a list of territory_ids representing the valid
            attack path; if no path exists, then it returns None instead
        '''
    ...
    def can_attack(self, source, target):
        ...
        Returns:
            bool: True if a valid attack path exists between
            source and target; else False
        '''
    ...
    def continent(self, continent_id):
        """
        Create a generator of all territories that belong to a
        given continent.
    ...
    def fortify(self, from_territory, to_territory, n_armies):
        ...
        Raises:
            ...
            ValueError if the territories
            do not share a border or are not owned by the same player.
        """
        if n_armies < 0 or self.armies(from_territory) <= n_armies:
            c = 'Board: Cannot move {n} armies from territory {tid}.'
            raise ValueError(c.format(n=n_armies, tid=from_territory))
        if to_territory not in [t.territory_id for t in self.friendly_neighbors(from_territory)]:
            a = 'Board: Cannot fortify, territories do not'
            b = 'share owner and/or border.'
    ...
    def attack(self, from_territory, to_territory, attackers):
        ...
        if attackers < 1 or self.armies(from_territory) <= attackers:
            d = 'Board: Cannot attack with {n} armies from territory {tid}.'
            raise ValueError(d.format(n=attackers, tid=from_territory))
        a = self.hostile_neighbors(from_territory)
        if to_territory not in [tid for (tid, _, _) in a]:
            b = 'Board: Cannot attack, territories do not share border ' 
            c = 'or are owned by the same player.'
            raise ValueError(a + b)
    ...
    def plot_board(self, path=None, plot_graph=False, filename=None):
        ...
        def plot_path(xs):
            ...
            for i, x in enumerate(xs[1:]):
                a = xs[i] == 1 and xs[i + 1] == 19
                if (xs[i] == 19 and xs[i + 1] == 1) or (a):
    ...
    def fight(cls, attackers, defenders):
        ...
        """
        n_atk_dice = min(attackers, 3)
        n_dfnd_dice = min(defenders, 2)
        C = cls.throw_dice()
    ...
    def mobile(self, player_id):
        """
        Create a generator of all territories of a player which can attack
        or move,
        ...
        """
        a = self.data
        return (t for t in a if (t.player_id == player_id and t.armies > 1))
